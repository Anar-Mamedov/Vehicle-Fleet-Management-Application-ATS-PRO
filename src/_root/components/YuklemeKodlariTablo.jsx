...
export function YuklemeKodlariTablo({ workshopSelectedId, onSubmit }) {
  ...
  const fetch = useCallback(
    async (diff = 0, targetPage = 1) => {
      ...
      try {
        ...
        if (diff > 0) {
          // Moving forward
          currentSetPointId = data[data.length - 1]?.seferYuklemeKodId || 0;
        } else if (diff < 0) {
          // Moving backward
          currentSetPointId = data[0]?.seferYuklemeKodId || 0;
        } else {
          currentSetPointId = 0;
        }
        const url = `ExpCode/GetExpCodeList?diff=${diff}&setPointId=${currentSetPointId}&parameter=${searchTerm}`;
  ...
  const handleModalOk = () => {
    ...
    if (selectedData) {
      setValue("yuklemeKodu", selectedData.yuklemeKod || "");
      setValue("yuklemeKodId", selectedData.key);
      setValue("yuklemeKodAciklama", selectedData.aciklama || "");
      setValue("yuklemeKodBirim", selectedData.birim || "");
