...
const FILTER_OPTIONS = [
  { value: "lokasyonIds", label: t("lokasyon") },
  { value: "otoyolKodIds", label: t("otoyol") },
  { value: "vIds", label: t("plaka") },
];

const fetchFilterData = async (filterKey) => {
  switch (filterKey) {
    case "lokasyonIds":
      return CustomCodeControlService("Location/GetLocationList").then((res) => res.data.map((item) => ({ value: item.locationId, label: item.locationName })));
    case "otoyolKodIds":
      return CodeControlService(901).then((res) => res.data.map((item) => ({ value: item.siraNo, label: item.codeText })));
    case "vIds":
