...
export default function ServisNedeni({ disabled, fieldRequirements }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get(`Code/GetCodeTextById?codeNumber=899`);
  ...
  const addItem = () => {
    if (name.trim() !== "") {
      if (options.some((option) => option.surucu === name)) {
        messageApi.open({
          type: "warning",
          content: "Bu ödeme durumu zaten var!",
        });
        return;
      }
      const body = {
          ...
          render={({ field }) => (
            <Select
              ...
              onChange={(value) => {
                ...
                setValue("odemeDurumu", value ?? null);
                setValue("odemeDurumuKodId", value ?? null);
