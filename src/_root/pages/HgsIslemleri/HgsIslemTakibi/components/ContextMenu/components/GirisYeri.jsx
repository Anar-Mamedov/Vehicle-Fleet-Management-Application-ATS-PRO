...
export default function ServisNedeni({ disabled, fieldRequirements }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await AxiosInstance.get(`Code/GetCodeTextById?codeNumber=896`);
  ...
  const addItem = () => {
    if (name.trim() !== "") {
      if (options.some((option) => option.surucu === name)) {
        messageApi.open({
          type: "warning",
          content: "Bu giriş yeri zaten var!",
        });
        return;
      }
      const body = {
          ...
          render={({ field }) => (
            <Select
              ...
              onChange={(value) => {
                ...
                setValue("girisYeri", value ?? null);
                setValue("girisYeriKodId", value ?? null);
