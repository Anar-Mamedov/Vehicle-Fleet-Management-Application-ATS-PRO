...
const GirisFisleri = () => {
    ...
    async (diff, targetPage) => {
      ...
      try {
        ...
        const response = await AxiosInstance.get(
          `WareHouse/GetWareHouseList?diff=${diff}&setPointId=${currentSetPointId}&parameter=${searchTerm}&type=MALZEME`
    ...
    () => [
      {
        ...
        sorter: (a, b) => {
          if (a.depoKod === null) return -1;
          if (b.depoKod === null) return 1;
          return a.depoKod.localeCompare(b.depoKod);
        },
      },
      {
        title: t("tanim"),
  ...
  const [columns, setColumns] = useState(() => {
    const savedOrder = localStorage.getItem("columnOrderMalzemeDepo1");
    const savedVisibility = localStorage.getItem("columnVisibilityMalzemeDepo1");
    const savedWidths = localStorage.getItem("columnWidthsMalzemeDepo1");
    ...
    localStorage.setItem("columnOrderMalzemeDepo1", JSON.stringify(order));
    localStorage.setItem("columnVisibilityMalzemeDepo1", JSON.stringify(visibility));
    localStorage.setItem("columnWidthsMalzemeDepo1", JSON.stringify(widths));
  ...
  useEffect(() => {
    localStorage.setItem("columnOrderMalzemeDepo1", JSON.stringify(columns.map((col) => col.key)));
    localStorage.setItem(
      "columnVisibilityMalzemeDepo1",
      JSON.stringify(
        columns.reduce(
          (acc, col) => ({
            ...acc,
  ...
  // Reset columns
  const resetColumns = () => {
    localStorage.removeItem("columnOrderMalzemeDepo1");
    localStorage.removeItem("columnVisibilityMalzemeDepo1");
    localStorage.removeItem("columnWidthsMalzemeDepo1");
